...
3. 输入```docker-compose up -d```进行安装，安装结束后利用命令```docker ps```查看，若容器都显示healthy，则表示安装成功 
...
1. 切换到juiceshop目录下，输入```docker-compose up -d```进行安装，安装结束后利用命令```docker ps```查看，若容器状态都显示healthy，则表示安装成功
...
2. 3000端口号只是用来访问juice shop的网页，burpsuite的端口号和浏览器代理设置的端口号要保持一致，但不需要设置成3000（设置3000会冲突，无法勾选）
